...
export const MistakeSlide = ({ slide }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.32 }} className="space-y-4 overflow-x-hidden" dir="rtl">
      <h3 className="text-2xl font-black text-blue-800 mb-4 [unicode-bidi:plaintext]">{slide.title}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-5 bg-white border-2 border-rose-100 rounded-2xl flex flex-col items-center justify-center text-center">
        ...
        {!isRevealed ? (
          <motion.button whileTap={{ y: 4 }} onClick={() => setIsRevealed(true)} className="p-5 bg-orange-500 text-white rounded-2xl flex items-center justify-center font-black text-lg shadow-sm py-4 px-6">
            اكتشف الجواب الصحيح ✨
          </motion.button>
        ) : (
          <motion.div initial={{ scale: 0.98 }} animate={{ scale: 1 }} className="p-5 bg-white border-2 border-orange-100 rounded-2xl flex flex-col items-center justify-center text-center">
